...
import { hashPassword, verifyPassword, canAccess, cookieName, cookieOptions, configured } from '../server/admin/core.mjs';
...
test('bootstrap, session expiry, revocation and persistent account checks', async () => {
  let time = 1000;const store = await createAdminStore(':memory:', () => time);
  try {
    ...
    const token = await store.createSession(user.id);assert.equal((await store.getSession(token)).id, user.id);
    assert.equal(await store.getSession(token.slice(1)), null);
    await store.revokeSession(token);assert.equal(await store.getSession(token), null);
    const expired = await store.createSession(user.id);time += 28800001;assert.equal(await store.getSession(expired), null);
...
test('persistent limiter enforces account and global limits and recovers after window', async () => {
  let time = 1000;const store = await createAdminStore(':memory:', () => time);
  try {
    for (let i = 0; i < 5; i++) assert(await store.consumeLogin(email));
    assert(!(await store.consumeLogin(email)));
    for (let i = 0; i < 95; i++) assert(await store.consumeLogin(`other${i}`));
    assert(!(await store.consumeLogin('new')));time += 900001;assert(await store.consumeLogin(email));
  } finally {await store.close();}
});
test('API validates CSRF, authentication, secure cookies, logout and fails closed', async () => {
  const store = await createAdminStore(':memory:');const deps = { store, env };
  try {
    await store.createOwner(email, password);
