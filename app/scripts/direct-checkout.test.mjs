...
async function fixture(t) {
  const store = await createAdminStore(':memory:');
  t.after(async () => await store.close());
  const plans = store.repository.plans;
  const rows = await Promise.all(['basico', 'intermediario', 'professional'].map(async (id, i) => {
    await plans.save(id, { ...(await plans.get(id)), monthly_price_cents: 10000 + i * 1000, cycles: 6 + i, active: true });
    return await plans.finish(await plans.start(id), 'provider_plan_' + i);
  }));
  const calls = [];
...
test('canonical codes select only their existing provider plan, without migrating IDs', async (t) => {
  const { plans, rows, calls, options } = await fixture(t);
...
test('inactive or unsynced plans display required message without contacting provider', async (t) => {
  ...
  for (const changes of [{ active: false }, { active: true }]) {
    const p = await plans.save('basico', { ...(await plans.get('basico')), ...changes });
...
test('API failure is sanitized and stale displayed revision is rejected', async (t) => {
  const { plans, rows, options } = await fixture(t);
