...
import { createAdminStore,cookieName,cookieOptions } from '../server/admin/core.mjs';
...
const env={NODE_ENV:'production',ADMIN_SITE_ORIGIN:'https://marquesano.com.br',ADMIN_DATABASE_PATH:process.cwd()+'/.admin-data/test-meta.sqlite',META_APP_ID:'123',META_APP_SECRET:'test-only-app-secret',META_REDIRECT_URI:'https://marquesano.com.br/api/admin/meta/callback'};
...
async function setup(){let time=Date.now();const store=createAdminStore(':memory:',()=>time);await store.createOwner('owner@example.com','test-meta-password-123');const user=await store.authenticate('owner@example.com','test-meta-password-123'),session=store.createSession(user.id),calls=[];return {store,user,session,calls,env,fetcher:fixtureFetcher(calls),advance:ms=>{time+=ms;}};}
...
test('OAuth real flow uses minimal scopes, Strict session bridge, encrypted storage, safe accounts and real campaigns',async()=>{
  const c=await setup();try{
    ...
    const row=c.store.repository.meta.get();assert(row.token_ciphertext.startsWith('v1.'));assert(!row.token_ciphertext.includes(token));assert(!JSON.stringify(row).includes(token));assert.equal(metaStatus(env,c.store.repository.meta).accountConnected,true);
    ...
    assert.equal((await api(c,'disconnect',{method:'POST',body:{}})).status,200);assert.equal(c.store.repository.meta.get().token_ciphertext,null);assert.equal(metaStatus(env,c.store.repository.meta).accountConnected,false);
...
test('state rejects wrong browser, replay, expiration, revoked sessions and changed role',async()=>{
  const c=await setup();try{
    ...
    const revoked=await start(c);c.store.revokeSession(c.session);assert((await(await callback(c,revoked)).text()).includes('STATE_INVALID'));
    c.session=c.store.createSession(c.user.id);const disabled=await start(c);
...
test('cancel and denied ads_read never overwrite an existing connection; errors never leak provider payload',async()=>{
  const c=await setup();try{
    await callback(c,await start(c));const before=c.store.repository.meta.get().token_ciphertext;
    const f=await start(c);const cancelled=await api(c,`callback?state=${f.state}&error=access_denied&error_description=raw-secret`,{browser:f.browser,session:undefined});const html=await cancelled.text();assert(html.includes('OAUTH_CANCELLED'));assert(!html.includes('raw-secret'));assert.equal(c.store.repository.meta.get().token_ciphertext,before);
    ...
    assert((await(await callback(c,await start(c))).text()).includes('PERMISSION_DENIED'));assert.equal(c.store.repository.meta.get().token_ciphertext,before);
    c.fetcher=async()=>Response.json({error:{code:190,message:'raw-secret '+token}},{status:400});
    const response=await api(c,'ad-accounts'),result=await response.json();assert.equal(response.status,409);assert.equal(result.code,'TOKEN_INVALID');assert(!JSON.stringify(result).includes(token));assert.equal(c.store.repository.meta.get().token_ciphertext,null);
  }finally{c.store.close();}
});
...
test('all Meta routes enforce OWNER/ADMIN, POST origin and read-only permissions',async()=>{
  const c=await setup();try{
    ...
    for(const role of ['MARKETING','SALES','VIEWER']){
      ...
      const session=c.store.createSession(saved.id);
...
test('configuration errors, ciphertext tampering, token expiry and disconnect invalidate authorization',async()=>{
  for(const [key,code]of [['META_APP_ID','APP_ID_MISSING'],['META_APP_SECRET','APP_SECRET_MISSING'],['META_REDIRECT_URI','REDIRECT_URI_MISSING']])assert.throws(()=>metaConfig({...env,[key]:''}),e=>e.code===code);
  assert.throws(()=>metaConfig({...env,META_REDIRECT_URI:'https://evil.example/api/admin/meta/callback'}),e=>e.code==='REDIRECT_URI_INVALID');
  const c=await setup();try{
    ...
    await callback(c,await start(c));c.advance(3600001);assert.equal(metaStatus(env,c.store.repository.meta).accountConnected,false);assert.throws(()=>metaAccess(c.store.repository.meta,env),e=>e.code==='TOKEN_INVALID');
    const encrypted=encryptToken(token,env);const fake={get:()=>({app_id:env.META_APP_ID,token_ciphertext:encrypted.slice(0,-6)+'broken',expires_at:Date.now()+10000}),now:Date.now};assert.throws(()=>metaAccess(fake,env),e=>e.code==='TOKEN_INVALID');
  }finally{c.store.close();}
});
...
test('pagination stays on graph.facebook.com and encryption survives repository recreation',async()=>{
  ...
  const db=new DatabaseSync(':memory:');try{
    db.exec('CREATE TABLE users(id TEXT PRIMARY KEY,email TEXT,role TEXT,active INTEGER);CREATE TABLE sessions(token_hash TEXT PRIMARY KEY,user_id TEXT REFERENCES users(id),expires INTEGER)');migrate(db);
    const hash=createHash('sha256').update('test-session').digest('hex');db.prepare('INSERT INTO users VALUES(?,?,?,?)').run('owner','owner@example.com','OWNER',1);db.prepare('INSERT INTO sessions VALUES(?,?,?)').run(hash,'owner',Date.now()+100000);
    const repo=createRepository(db);assert(repo.meta.save({session_hash:hash,revision:0},{appId:env.META_APP_ID,ciphertext:encryptToken(token,env),expiresAt:Date.now()+100000,profile}));
    assert.equal(metaStatus(env,createRepository(db).meta).accountConnected,true);
    assert(!repo.meta.save({session_hash:hash,revision:0},{appId:env.META_APP_ID,ciphertext:'stale',expiresAt:0,profile}));
  }finally{db.close();}
});
...
test('proxy permits only the GET OAuth callback without the Strict cookie, and protects every other Meta route',async()=>{
  ...
  assert.equal(proxy(request('callback?state=test')).headers.get('x-middleware-next'),'1');
  for(const route of ['connect','status','ad-accounts','campaigns','disconnect'])assert.equal(proxy(request(route)).status,401);
  assert.equal(proxy(request('callback',{method:'POST'})).status,401);
  assert.equal(proxy(request('callback',{method:'HEAD'})).status,401);
  user={role:'MARKETING'};assert.equal(proxy(request('ad-accounts')).status,403);
  user={role:'OWNER'};assert.equal(proxy(request('connect')).headers.get('x-middleware-next'),'1');
  assert.equal(proxy(request('callback')).headers.get('Referrer-Policy'),'no-referrer');
});
...
test('no ad accounts is an honest connected state; optional business permission does not block ads_read',async()=>{
  const c=await setup();try{
    ...
    assert.equal(metaStatus(env,c.store.repository.meta).accountConnected,true);
    assert.equal(metaStatus(env,c.store.repository.meta).adAccountSelected,false);
...
test('disconnect during code exchange cannot restore the connection; expiry and revoked grants never expose tokens',async()=>{
  const c=await setup();try{
    ...
    c.fetcher=async(url,options)=>{if(!cleared&&url.includes('/oauth/access_token')){cleared=true;c.store.repository.meta.clear(c.store.repository.meta.get().revision);}return original(url,options);};
    assert((await(await callback(c,flow)).text()).includes('CONNECTION_CHANGED'));
    assert.equal(c.store.repository.meta.get().token_ciphertext,null);
  }finally{c.store.close();}
});
