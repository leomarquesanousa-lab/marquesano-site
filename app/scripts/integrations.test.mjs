...
test('Meta status never returns secrets or claims that credentials are an authorized connection',()=>{
  ...
  const status=metaStatus(env);assert.equal(status.appConfigured,true);assert.equal(status.accountConnected,false);assert.equal(status.adAccountSelected,false);assert.equal(status.oauthReady,true);
  assert(Object.values(status.capabilities).every(value=>value===false));
  assert(!JSON.stringify(status).includes('private-'));
  assert.equal(metaStatus({...env,META_REDIRECT_URI:'https://user:private@host/'}).redirectConfigured,false);
  assert.equal(metaStatus({}).status,'Não conectado');
});
...
test('audit filters match totals, last login derives from audit and local traffic counts visits',async()=>{
  const store=createAdminStore(':memory:');const repo=store.repository;
  try{
    ...
    repo.audit(actor,'login','auth',actor.id);
    assert(repo.list('usuarios',new URLSearchParams(),period).rows[0].last_login);
    repo.audit(null,'login','auth','test-login');repo.audit(null,'settings','configuracoes','test-setting');
    const filtered=repo.list('auditoria',new URLSearchParams('action=login&entity=auth&q=Sistema'),period);
    assert.equal(filtered.total,1);assert.equal(filtered.rows.length,1);assert.equal(filtered.rows[0].action,'login');
    assert.equal(repo.list('auditoria',new URLSearchParams('action=login&entity=configuracoes'),period).total,0);
    const id=repo.recordEvent(null,{id:crypto.randomUUID(),name:'page_view',path:'/'},{});
    repo.recordEvent(id,{id:crypto.randomUUID(),name:'page_view',path:'/planos'},{});
    const report=repo.report(period);assert.equal(report.traffic[0].visitors,1);assert.equal(report.traffic[0].views,2);assert.equal(report.counts.forms,0);
  }finally{store.close();}
});
