...
const env={NODE_ENV:'production',ADMIN_SITE_ORIGIN:'https://marquesano.com.br',ADMIN_DATABASE_PATH:process.cwd()+'/.admin-data/unused.sqlite',MERCADOPAGO_ACCESS_TOKEN:'mock-access-token',MERCADOPAGO_WEBHOOK_SECRET:'mock-webhook-secret'};
...
function fixture(t) {
  ...
  const plan=store.repository.plans.get('basico');store.repository.plans.save('basico',{...plan,mercadopago_plan_id:'mp_plan_basic'});
...
test('duplicate events and concurrent deliveries do not duplicate subscription/payment/event records',async t=>{
  ...
  assert.equal(report().totals.subscriptions,1);assert.equal(report().totals.events,1);
  await send('payment',101);await send('payment',101,{requestId:'retry-3'});await send('subscription_authorized_payment',201);
  assert.equal(report().totals.payments,1);assert.equal(report().totals.invoices,1);assert.equal(report().totals.events,3);
});

test('creation, authorization, pause, cancellation and next billing date come from the API',async t=>{
  const {send,state,report}=fixture(t);
  for(const [i,status]of ['pending','authorized','paused','cancelled'].entries()) {
    ...
    assert.equal(report().subscriptions[0].status,status);assert.equal(report().subscriptions[0].next_payment_date,status==='cancelled'?null:'2026-10-20T10:00:00.000Z');
  }
  state.subscription.status='authorized';state.subscription.last_modified=stamp;await send();assert.equal(report().subscriptions[0].status,'cancelled');
});

test('approved payment confirmed from payment API, not invoice/body status',async t=>{
  const {send,state,report}=fixture(t);state.invoice.payment.status='rejected';
  assert.equal((await send('subscription_authorized_payment',201,{body:{status:'rejected'}})).status,200);
  const saved=report().payments[0];assert.equal(saved.status,'approved');assert.equal(saved.amount_cents,9900);assert.equal(saved.id,'101');assert.equal(saved.invoice_id,'201');assert.equal(saved.subscription_id,'sub_abc');
...
test('pending and rejected payments; stale notification cannot regress a newer payment',async t=>{
  ...
  for(const [i,status]of ['pending','rejected','approved'].entries()) {
    ...
    assert.equal((await send('payment',101)).status,200);assert.equal(report().payments[0].status,status);
  }
  state.payment.status='pending';state.payment.date_last_updated=stamp;await send('payment',101);
  assert.equal(report().payments[0].status,'approved');assert.equal(report().totals.payments,1);
});

test('payment before invoice is linked later without duplicating it; invoice without payment is not approval',async t=>{
  const {send,state,report}=fixture(t);state.search=false;
  await send('payment',101);assert.equal(report().payments[0].subscription_id,null);
  await send('subscription_authorized_payment',201);assert.equal(report().payments[0].subscription_id,'sub_abc');assert.equal(report().totals.payments,1);
  ...
  assert.equal(report().totals.payments,1);assert.equal(report().invoices[0].status,'scheduled');
});

test('invalid signature, mismatched signed ID, malformed input and missing secret fail closed',async t=>{
  const {store,fetcher,state,send,report}=fixture(t);
  ...
  assert.equal(state.calls.length,0);assert.equal(report().totals.events,0);
});

test('provider failure is retryable and sanitized; wrong collector rejected; unknown plan ignored',async t=>{
  const {send,state,report,store}=fixture(t);const originalPlans=store.repository.plans.list();state.fail=true;
  const failed=await send();assert.equal(failed.status,503);assert(!(await failed.text()).includes(env.MERCADOPAGO_ACCESS_TOKEN));assert.equal(report().totals.events,0);
  state.fail=false;state.subscription.collector_id=43;assert.equal((await send()).status,403);
  state.subscription.collector_id=42;state.subscription.preapproval_plan_id='other-plan';assert.equal((await (await send()).json()).ignored,true);assert.equal(report().totals.subscriptions,0);
  state.subscription.preapproval_plan_id='mp_plan_basic';assert.equal((await send()).status,200);
  assert.deepEqual(store.repository.plans.list(),originalPlans);
});
...
test('public webhook uses signed headers, accepts lowercase-normalized IDs and delayed retries without admin session',async t=>{
  const {send,report}=fixture(t);assert.equal((await send('subscription_preapproval','SUB_ABC',{ts:'1704908010'})).status,200);assert.equal(report().totals.subscriptions,1);
});

test('admin history is restricted to OWNER/ADMIN, paginated, and never reveals credentials',async t=>{
  const {store,send,fetcher}=fixture(t);await send();await send('payment',101);
