...
export default function CardCheckout({ code, revision, amount, publicKey, diagnostics = false }) {
  ...
  async function send(token) {
    ...
    try {
      ...
      setError(data.error || 'Não foi possível confirmar a assinatura. Tente verificar novamente.');
    } catch {
  ...
  return <>
    ...
    <form ref={form} id="marquesano-card-form" className={styles.form} onSubmit={event => event.preventDefault()}>
      ...
      {verify ? <button className={styles.button} type="button" disabled={loading} onClick={() => void send()}>{loading ? 'Verificando…' : 'Verificar assinatura'}</button> : <button className={styles.button} type="submit" disabled={!ready || loading} aria-busy={loading}>{loading ? 'Confirmando assinatura…' : ready ? `Assinar por ${priceLabel(amount)} / mês` : 'Carregando pagamento seguro…'}</button>}
