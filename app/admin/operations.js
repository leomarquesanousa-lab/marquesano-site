...
export default function Operations({module,user,recordId}) {
  ...
  return <>
    ...
    {!['configuracoes','usuarios'].includes(module)&&<form className={styles.filters} onSubmit={e=>e.preventDefault()}>
      ...
      {module==='auditoria'&&<><label>Usuário<input placeholder="E-mail ou Sistema" value={filters.q} maxLength={100} onChange={e=>updateFilter('q',e.target.value)}/></label><label>Ação<select value={filters.action||''} onChange={e=>updateFilter('action',e.target.value)}>{[['','Todas'],['login','Entrada'],['logout','Saída'],['create','Criação'],['update','Edição'],['settings','Configuração'],['convert','Conversão'],['note','Nota']].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label>Entidade<select value={filters.entity||''} onChange={e=>updateFilter('entity',e.target.value)}>{[['','Todas'],['auth','Autenticação'],['usuarios','Usuários'],['configuracoes','Configurações'],['leads','Leads'],['clientes','Clientes'],['campanhas','Campanhas']].map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label></>}
