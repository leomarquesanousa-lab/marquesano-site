...
export async function migratePlans(db) {
  await db.exec(`CREATE TABLE subscription_plans (
    id TEXT PRIMARY KEY, name TEXT NOT NULL, description TEXT NOT NULL,
    monthly_price_cents BIGINT, cycles BIGINT NOT NULL, active BIGINT NOT NULL,
    mercadopago_plan_id TEXT UNIQUE, revision BIGINT NOT NULL DEFAULT 1,
    synced_revision BIGINT, sync_state TEXT NOT NULL DEFAULT 'pending', sync_started BIGINT,
    CHECK(monthly_price_cents IS NULL OR monthly_price_cents>0), CHECK(cycles>0), CHECK(active IN (0,1))
  )`);
  await ensurePlans(db);
}

export async function ensurePlans(db) {
  const insert = db.prepare('INSERT INTO subscription_plans(id,name,description,monthly_price_cents,cycles,active) VALUES(?,?,?,?,?,?) ON CONFLICT(id) DO NOTHING');
  for (const p of initialPlans) await insert.run(p.id, p.name, p.description, p.monthly_price_cents, p.cycles, Number(p.active));
}

export function createPlanStore(db, now = Date.now) {
  const get = async (id) => {
    const row = await db.prepare('SELECT * FROM subscription_plans WHERE id=?').get(id);
  ...
  return {
    get,
    list: async () => await Promise.all(initialPlans.map(async (p) => await get(p.id))),
    async save(id, data) {
      ...
      if (remote && (await db.prepare('SELECT id FROM subscription_plans WHERE mercadopago_plan_id=? AND id<>?').get(remote, id))) throw new InputError('Esse ID já pertence a outro plano.');
      const uncertain = ['syncing', 'unknown'].includes(old.sync_state) && !old.mercadopago_plan_id && !remote;
      const changed = await db.prepare("UPDATE subscription_plans SET name=?,description=?,monthly_price_cents=?,cycles=?,active=?,mercadopago_plan_id=?,revision=revision+1,sync_state=?,sync_started=NULL WHERE id=? AND revision=? AND (sync_state<>'syncing' OR sync_started<=?)").
    ...
    async start(id) {
      ...
      const result = await db.prepare("UPDATE subscription_plans SET sync_state='syncing',sync_started=? WHERE id=? AND revision=? AND (sync_state<>'syncing' OR sync_started<=?)").run(now(), id, p.revision, now() - 60000);
      if (!result.changes) throw new InputError('Sincronização já em andamento.', 409);
      return p;
    },
    async finish(p, remote) {
      await db.prepare("UPDATE subscription_plans SET mercadopago_plan_id=?,synced_revision=?,sync_state='synced',sync_started=NULL WHERE id=? AND revision=?").run(remote, p.revision, p.id, p.revision);
      return await get(p.id);
    },
    async fail(p, uncertain) {
      await db.prepare('UPDATE subscription_plans SET sync_state=?,sync_started=NULL WHERE id=? AND revision=?').run(uncertain ? 'unknown' : 'error', p.id, p.revision);
    }
  };
}
