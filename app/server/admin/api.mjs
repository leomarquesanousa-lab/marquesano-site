import { adminStore, configured, cookieName, cookieOptions, canAccess } from './core.mjs';
...
export async function handleAdminApi(request, path, dependencies = {}) {
  ...
  if (!configured(env)) return json({ error: 'Acesso administrativo indisponível.' }, 503);
  const action = path.join('/');
  try {
    const store = dependencies.store || adminStore();
    ...
    if (action === 'login' && request.method === 'POST') {
      ...
      if (!store.consumeLogin(email)) { const response = json({ error: 'Muitas tentativas. Aguarde 15 minutos.' }, 429); response.headers.set('Retry-After', '900'); return response; }
      const user = await store.authenticate(email, data.password);
      if (!user) return json({ error: 'Credenciais inválidas.' }, 401);
      store.repository.audit(user,'login','auth',user.id);
      store.revokeSession(token);
      const response = json({ ok: true });
      response.cookies.set(cookieName(env), store.createSession(user.id), cookieOptions(env));
      return response;
    }
    const user = store.getSession(token);
    if (!user) return json({ error: 'Autenticação necessária.' }, 401);
    if (action === 'logout' && request.method === 'POST') {
      store.repository.audit(user,'logout','auth',user.id);
      store.revokeSession(token);
      const response = json({ ok: true });
