...
export async function handleMetaApi(request,path,store,env=process.env,{fetcher=fetch}={}) {
  ...
  if(action==='callback'&&request.method==='GET') {
    try {
      ...
      const flow=meta.consume(state,browser);
      if(!flow)throw new MetaError('STATE_INVALID');
      const current=request.cookies.get(cookieName(env))?.value;
      if(current&&store.getSession(current)?.id!==flow.user.id)throw new MetaError('STATE_INVALID');
      ...
      if(!meta.save(flow,{appId:config.appId,ciphertext:encryptToken(token.access_token,env),expiresAt,profile}))throw new MetaError('CONNECTION_CHANGED');
      store.repository.audit(flow.user,'connect','meta',profile.id);
  ...
  const sessionToken=request.cookies.get(cookieName(env))?.value,user=store.getSession(sessionToken);
  ...
  try {
    if(action==='connect'&&request.method==='GET') {
      ...
      meta.begin(state,browser,sessionToken);
    ...
    if(action==='status'&&request.method==='GET')return json(metaStatus(env,meta));
    if(action==='disconnect'&&request.method==='POST') {
      await readJson(request);const row=meta.get();
      if(!meta.clear(row.revision))throw new MetaError('CONNECTION_CHANGED',409);
      store.repository.audit(user,'disconnect','meta','connection');
      ...
      return json({ok:true,message:'Meta desconectada deste painel.',meta:metaStatus(env,meta)});
    }
    ...
    const access=metaAccess(meta,env,fetcher);revision=access.row.revision;
    ...
    if(action==='test') {
      ...
      const selected=metaStatus(env,meta).selectedAccount;
      if(selected&&!accounts.accounts.some(a=>a.id===selected.id))await access.client.get(selected.id,{fields:'id'});
      if(meta.get().revision!==revision)throw new MetaError('CONNECTION_CHANGED',409);
      return json({ok:true,message:accounts.accounts.length?'Conexão Meta validada.':'Conexão Meta validada, mas nenhuma conta de anúncios foi encontrada.',profile,...accounts,meta:metaStatus(env,meta)});
    }
    ...
    if(!meta.select(account,revision))throw new MetaError('CONNECTION_CHANGED',409);
    store.repository.audit(user,'select','meta',account.id);
    return json({ok:true,meta:metaStatus(env,meta)});
  } catch(error) {
    const safe=safeMetaError(error);
    if(safe.code==='TOKEN_INVALID'&&revision!==undefined)meta.clear(revision);
