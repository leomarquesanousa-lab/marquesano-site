...
export function metaStatus(env=process.env,store) {
  let config,error;try{config=metaConfig(env);}catch(e){error=safeMetaError(e);}
  const row=store?.get(),valid=Boolean(config&&row?.token_ciphertext&&row.app_id===config.appId&&row.expires_at>store.now());
...
export function metaAccess(store,env=process.env,fetcher=fetch) {
  metaConfig(env);const row=store.get();
...
export async function metaCampaigns(store,period,env=process.env,fetcher=fetch) {
  const {row,client}=metaAccess(store,env,fetcher),account=parse(row.account_json);
  ...
  if(store.get().revision!==row.revision)throw new MetaError('CONNECTION_CHANGED',409);
