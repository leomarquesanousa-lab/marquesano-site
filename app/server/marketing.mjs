...
export function contactPersistence() {
  return {
    async capture(clean, key, request, data) {
      ...
      return await (await adminStore()).repository.captureContact(clean, key, cookie, touch);
    },
    async sent(record, id) {if (record) await (await adminStore()).repository.emailStatus(record.id, 'SENT', id);},
    async failed(record) {if (record) await (await adminStore()).repository.emailStatus(record.id, 'FAILED');},
    async unknown(record) {if (record) await (await adminStore()).repository.emailStatus(record.id, 'UNKNOWN');}
  };
}
export async function marketingRequest(request) {
  ...
  try {
    ...
    const repo = (await adminStore()).repository,settings = await repo.settings();
    ...
    const visitor = await repo.recordEvent(request.cookies.get(visitorCookie())?.value, { id: data.id, name: data.name, path }, attribution(data.attribution));
